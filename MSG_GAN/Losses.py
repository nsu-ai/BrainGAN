...
class RelativisticAverageHingeGAN(GANLoss):
    ...
    def __init__(self, device, dis):
        ...
        self.aux_loss = th.nn.CrossEntropyLoss().to(device)

    def dis_loss(self, real_samps, fake_samps):
        # difference between real and fake:
        re, _ = self.dis(real_samps)
        fe, _ = self.dis(fake_samps)
        r_f_diff = re - th.mean(fe)

        # difference between fake and real samples
        f_r_diff = fe - th.mean(re)
    ...
    def conditional_dis_loss(self, real_samps, fake_samps, fake_labels, labels): #conditional_vectors):
        # difference between real and fake:
        real_pred, real_aux = self.dis(real_samps)
        fake_pred, fake_aux = self.dis(fake_samps)
        
        real_aux_loss = self.aux_loss(real_aux, labels)
        fake_aux_loss = self.aux_loss(fake_aux, fake_labels)

        real_ad_loss = ((real_pred - th.mean(fake_pred)) + (real_aux_loss - th.mean(fake_aux_loss))) / 2
        fake_ad_loss = ((fake_pred - th.mean(real_pred)) + (fake_aux_loss - th.mean(real_aux_loss))) / 2

        # return the loss
        return (th.mean(th.nn.ReLU()(1 - real_ad_loss))
                + th.mean(th.nn.ReLU()(1 + fake_ad_loss)))

    def conditional_gen_loss(self, real_samps, fake_samps, fake_labels, labels):#, conditional_vectors):
        # difference between real and fake:
        real_pred, real_aux = self.dis(real_samps)
        fake_pred, fake_aux = self.dis(fake_samps)
...
class CustomHingeGAN(GANLoss):
    def __init__(self, device, dis):
        super().__init__(device, dis)

        self.ad_loss = th.nn.BCEWithLogitsLoss().to(device)
        self.aux_loss = th.nn.CrossEntropyLoss().to(device)
    ...
    def conditional_dis_loss(self, real_samps, fake_samps, fake_labels, labels): #conditional_vectors):
        ...
        real_pred, real_aux = self.dis(real_samps)
        fake_pred, fake_aux = self.dis(fake_samps)
        real_ad_loss = self.ad_loss(real_pred,
                                   th.ones(real_pred.shape[0]).to(self.device))
        fake_ad_loss = self.ad_loss(fake_pred,
                                   th.zeros(fake_pred.shape[0]).to(self.device))
        
        real_aux_loss = self.aux_loss(real_aux, labels)
        fake_aux_loss = self.aux_loss(fake_aux, fake_labels)

        real_d_loss = (real_ad_loss + real_aux_loss) / 2
        fake_d_loss = (fake_ad_loss + fake_aux_loss) / 2

        # return the loss
        return (real_d_loss + fake_d_loss) / 2
    ...
    def conditional_gen_loss(self, real_samps, fake_samps, fake_labels, labels):#, conditional_vectors):
        ...
        fake_ad_loss = self.ad_loss(fake_pred,
                              th.ones(fake_pred.shape[0]).to(self.device))
        fake_aux_loss = self.aux_loss(fake_aux, fake_labels)

        # real_d_loss = (real_ad_loss + real_aux_loss) / 2
        fake_d_loss = (fake_ad_loss + fake_aux_loss) * 0.5
