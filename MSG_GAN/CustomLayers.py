...
class CustomFinalBlock(th.nn.Module):
    """ Final block for the Discriminator """
    ...
    def __init__(self, in_channels, n_classes=4):
        ...
        # final conv layer emulates a fully connected layer
        self.conv_3 = Linear(self.in_features, 1) # Conv2d(in_channels, 1, (1, 1), bias=True)
        self.aux = Linear(self.in_features, self.n_classes) # Conv2d(in_channels, n_classes, (1, 1), bias=True)
    ...
    def forward(self, x):
        ...
        # fully connected layer
        out1 = self.conv_3(y)  # This layer has linear activation
        
        out2 = self.aux(y)
        # print(out2.shape)
        # flatten the output raw discriminator scores
        return out1.view(-1), out2#.view(-1, self.n_classes)
