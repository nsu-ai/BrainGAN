...
class Generator(th.nn.Module):
    ...
    def __init__(self, depth=7, latent_size=512, dilation=1, use_spectral_norm=True):
        ...
        # self.l1 = nn.Linear(self.latent_size, self.latent_size)
...
class CustomGenerator(th.nn.Module):
    """ Generator of the GAN network """

    def __init__(self, depth=7, latent_size=512, dilation=1, use_spectral_norm=True, n_classes=4):
        """
        constructor for the Generator class
        :param depth: required depth of the Network
        :param latent_size: size of the latent manifold
        ...
        :param use_spectral_norm: whether to use spectral normalization
        """
        from torch.nn import ModuleList, Conv2d, Embedding, Sequential, Linear
        from MSG_GAN.CustomLayers import GenGeneralConvBlock, GenInitialBlock
        ...
        self.label_emb = Embedding(n_classes, self.latent_size)
        # self.init_size = 128 // 4  # Initial size before upsampling
        # self.l1 = nn.Sequential(nn.Linear(self.latent_size, 128 * self.init_size ** 2))
        # self.l1 = Linear(latent_size, 256)

        # self.label_emb = Sequential(self.label_emb, self.l1)
    ...
    def forward(self, x, labels):
        ...
        #print(lab.shape)
        y = th.mul(lab, y)
        #print(y.shape)  # start the computational pipeline
        for block, converter in zip(self.layers, self.rgb_converters):
            y = block(y)
            out = converter(y)
            outputs.append(tanh(out))
...
class CustomDiscriminator(th.nn.Module):
    """ Discriminator of the GAN """

    def __init__(self, depth=7, feature_size=512, dilation=1, use_spectral_norm=True, n_classes=4):
        """
        ...
        from MSG_GAN.CustomLayers import DisGeneralConvBlock, DisFinalBlock, CustomFinalBlock
        ...
        # create a module list of the other required general convolution blocks
        self.layers = ModuleList([CustomFinalBlock(self.feature_size, self.n_classes)])
...
class Custom_MSG_GAN:
    """ Conditional TeacherGAN
    ...
    def __init__(self, depth=7, latent_size=512, gen_dilation=1,
                 dis_dilation=1, use_spectral_norm=True, device=th.device("cpu"), n_classes=4):
        """ constructor for the class """
        from torch.nn import DataParallel

        self.gen = CustomGenerator(depth, latent_size, dilation=gen_dilation,
                             use_spectral_norm=use_spectral_norm, n_classes=n_classes).to(device)
        self.dis = CustomDiscriminator(depth, latent_size, dilation=dis_dilation,
                                 use_spectral_norm=use_spectral_norm, n_classes=n_classes).to(device)
        ...
        self.aux_loss = th.nn.CrossEntropyLoss().to(self.device)
    ...
    def generate_samples(self, num_samples):
        ...
        """
        noise = th.autograd.Variable(th.FloatTensor(np.random.normal(0, 1, (num_samples, self.latent_size)))).to(self.device)

        labels = th.autograd.Variable(th.LongTensor(np.random.randint(0, self.n_classes, num_samples))).to(self.device)
        
        generated_images = self.gen(noise, labels)[-1]
    ...
    def optimize_discriminator(self, dis_optim, noise, real_batch, loss_fn, gen_labels, labels):
        """
        ...
        fake_samples = self.gen(noise, gen_labels)
        
        fake_samples = list(map(lambda x: x.detach(), fake_samples))
        
        loss = loss_fn.conditional_dis_loss(real_batch, fake_samples, gen_labels, labels)
        
        #classes = th.FloatTensor(classes.cpu().type(th.FloatTensor)).to(self.device)#.type(th.Float32)
        
        #labels = th.LongTensor(labels.cpu().type(th.LongTensor)).to(self.device)#.type(th.Float32)

        # print(classes.shape, labels.shape)
        #aux_loss = self.aux_loss(classes.view(-1, self.n_classes), labels.view(-1))

        #loss = 0.5 * (loss + aux_loss)
    ...
    def optimize_generator(self, gen_optim, noise, real_batch, loss_fn, gen_labels, labels):
        """
        ...
        loss = loss_fn.conditional_gen_loss(real_batch, fake_samples, gen_labels, labels)
        
        #classes = th.FloatTensor(classes.cpu().type(th.FloatTensor)).to(self.device)#.type(th.Float32)
        
        #labels = th.LongTensor(labels.cpu().type(th.LongTensor)).to(self.device)#.type(th.Float32)
        #aux_loss = self.aux_loss(classes, labels)
    ...
    def create_grid(samples, img_files):
        ...
        sample = th.clamp((samples.detach() / 2) + 0.5, min=0, max=1)
        save_image(sample, img_files[0], nrow=int(sqrt(samples.shape[0])))
    ...
    def train(self, data, gen_optim, dis_optim, loss_fn,
        ...
        fixed_input = th.FloatTensor(np.random.normal(0, 1, (num_samples, self.latent_size))).to(self.device)
        fixed_labels = th.LongTensor(np.random.randint(0, self.n_classes, num_samples)).to(self.device)
        ...
        for epoch in range(start, num_epochs + 1):
            ...
            for (i, batch) in enumerate(data):
                ...
                gan_input = th.autograd.Variable(th.FloatTensor(np.random.normal(0, 1, (extracted_batch_size, self.latent_size)))).to(self.device)

                gen_labels = th.autograd.Variable(th.LongTensor(np.random.randint(0, self.n_classes, extracted_batch_size))).to(self.device)
                # print(labels.shape, gen_labels.shape)
                # optimize the discriminator:
                dis_loss = self.optimize_discriminator(dis_optim, gan_input,
                                                       images, loss_fn, gen_labels=gen_labels, labels=labels)
                ...
                if i % int(feedback_factor) == 0 or i == 1:
                    ...
                    gen_optim.zero_grad()
                    with th.no_grad():
                        gens = self.gen(fixed_input, fixed_labels)[-1]
                        self.create_grid(gens, gen_img_files)
...
class MSG_GAN:
    ...
    def generate_samples(self, num_samples):
        ...
        generated_images = self.gen(noise)[-1]
    ...
    def train(self, data, gen_optim, dis_optim, loss_fn,
        ...
        for epoch in range(start, num_epochs + 1):
            ...
            for (i, batch) in enumerate(data, 1):
                ...
                gan_input = th.randn(
                    extracted_batch_size, self.latent_size).to(self.device)
                gan_inout = th.autograd.Variable(th.FloatTensor(np.random.normal(0, 1, (extracted_batch_size, self.latent_size)))).to(self.device)
